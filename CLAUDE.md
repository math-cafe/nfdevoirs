...
- Architecture modulaire avec 14 modules spécialisés
...
Le projet utilise une **architecture modulaire** avec 14 modules spécialisés :

### Structure générale
- `nfdevoirs.cls` - Classe LaTeX principale chargeant tous les modules
- `tests/test-simple.tex` - Document de test principal avec exemples DM et QCM
...
### Environnement QCM
```latex
\begin{qcm}{points=3, bonus=1, style=num, col=2, niveau=4}
...
- `niveau=N` - Difficulté 1-5 étoiles (défaut: 1)
- `style=case|num|mix` - Style d'affichage (défaut: case)
